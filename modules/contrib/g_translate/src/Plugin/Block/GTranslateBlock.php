...
namespace Drupal\g_translate\Plugin\Block;
...
class GTranslateBlock extends BlockBase {
    ...
    public function build() {
        $settings = \Drupal::config('g_translate.settings');
...
EOT;
            ...
            switch($gtranslate_look) {
                case 'flags_dropdown': {
                    $block_content .= '<style>'."\n";
                    $block_content .= "a.gtflag {background-image:url('".base_path().\Drupal::service('extension.list.module')->getPath('g_translate')."/gtranslate-files/".$gtranslate_flag_size."a.png');}\n";
                    $block_content .= "a.gtflag:hover {background-image:url('".base_path().\Drupal::service('extension.list.module')->getPath('g_translate')."/gtranslate-files/".$gtranslate_flag_size.".png');}\n";
                    $block_content .= '</style>';
                    ...
                    foreach($languages as $lang => $lang_name) {
                        if($settings->get('gtranslate_'.$lang)) {
                            ...
                            $block_content .= '<option value="'.$gtranslate_main_lang.'|'.$lang.'" style="'.($lang == $gtranslate_main_lang ? 'font-weight:bold;' : '').'background:url(\''.base_path().\Drupal::service('extension.list.module')->getPath('g_translate').'/gtranslate-files/16l.png\') no-repeat scroll 0 -'.$flag_y.'px;padding-left:18px;">'.$lang_name.'</option>';
                        }
                ...
                case 'dropdown_with_flags': {
                    ...
                    $block_content .= '<a href="#" onclick="return false;"><span class="gflag" style="background-position:-'.$flag_x.'px -'.$flag_y.'px"><img src="'.base_path().\Drupal::service('extension.list.module')->getPath('g_translate').'/gtranslate-files/blank.png" height="16" width="16" alt="'.$languages[$current_language].'" /></span>'.$languages[$current_language].'</a>';
                    $block_content .= '</div>';
                    ...
                    // Adding slider css
                    $module_url = base_path().\Drupal::service('extension.list.module')->getPath('g_translate').'/gtranslate-files';
                    $block_content .= "<style>
            ...
            switch($gtranslate_look) {
                ...
                case 'dropdown_with_flags': {
                    ...
                    foreach($languages as $lang => $lang_name) {
                        ...
                        if($settings->get('gtranslate_'.$lang) == '2')
                            $block_content .= '<a href="'.$href.'" title="'.$lang_name.'" class="nturl '.($current_language == $lang ? ' selected' : '').'"><span class="gflag" style="background-position:-'.$flag_x.'px -'.$flag_y.'px;"><img src="'.base_path().\Drupal::service('extension.list.module')->getPath('g_translate').'/gtranslate-files/blank.png" height="16" width="16" alt="'.$lang_name.'" /></span>'.$lang_name.'</a>';
                    }
        ...
        if(isset($jquery_slider))
            $return['#attached']['library'][] = 'g_translate/jquery-slider';
